...
export default function VoiceStep() {
  ...
  const handlePlayPreview = (readerId: string) => {
    ...
    // Play Al-Fatiha first verse as a preview
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2 mb-2">
        <Volume2 className="w-5 h-5 text-qudra" />
      ...
      <div className="grid grid-cols-3 gap-2">
        <button
          ...
          className={`flex flex-col items-center gap-2 rounded-xl border p-4 transition-all ${
            audioSource === 'reader'
              ? 'border-qudra bg-qudra/5 text-qudra'
              : 'border-border hover:border-qudra/30 text-muted-foreground'
          }`}
        >
      ...
      {audioSource === 'upload' && (
        <div className="surface rounded-xl p-8 text-center">
          ...
          <Button variant="outline" className="border-qudra/30 text-qudra hover:bg-qudra/10">
      ...
      {audioSource === 'reader' && (
        <>
          ...
          {selectedReader && (
            <div className="bg-qudra/5 border border-qudra/20 rounded-xl p-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-qudra" />
                ...
                <Badge
                  variant="outline"
                  className="text-[10px] border-qudra/30 text-qudra"
                >
              ...
              {filteredReaders.map((reader) => (
                <div
                  ...
                  className={`flex items-center gap-3 rounded-lg p-3 cursor-pointer transition-all ${
                    selectedReader?.id === reader.id
                      ? 'bg-qudra/5 border border-qudra/20'
                ...
                >
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 text-muted-foreground hover:text-qudra flex-shrink-0"
                    onClick={(e) => {
                      e.stopPropagation();
